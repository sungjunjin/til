...
GC의 대상이 되는 경우는 다음과 같다
- 객체가 null인 경우
- 블록 실행 종료 후, 블록 안에서 생성된 객체
- 부모 객체가 null인 경우의 자식 객체
...
- 다음 Minor GC가 발생할 때, Survivor 영역에서 살아남은 객체들은 옆에 있는 Survivor 영역으로 이동한다. 이 때 Eden 영역에 살아남은 객체들도 해당 Survivor 영역으로 이동한다 이 과정이 반복되면서 둘 중 하나의 Survivor 영역은 무조건 빈 상태가 된다

2. Old 영역
- Survivor 영역에서 일정 기간동안 살아남은 객체들은 최종적으로 Old 영역에 저장된다
