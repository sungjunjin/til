...
템플릿 메소드 패턴이란 여러 클래스에서 공통적으로 사용되는 메소드를 템플릿화 하여 상위 클래스에 정의하고, 하위 클래스에서 세부 동작을 다르게 구현하는 패턴이다.
...
public class Cup {
    ...
    public Cup(int sizeMl) {
        this.sizeMl = sizeMl;
    }

    public static Cup of(CupSize size) {
        return switch (size) {
...
팩토리 메소드는 객체 생성을 생성자가 아닌, 객체에 대한 생성을 담당하는 별도의 메소드로 구성하여 객체를 생성하는 메소드이다. 클라이언트가 내부 구성을 자세하게 몰라도 객체를 생성할 수 있다는 장점이 있다. 위와 같은 예시는 **정적 팩토리 메소드**로써 private 생성자와 객체를 생성하는 정적 메소드로 구성되어 있다.

팩토리 메소드의 또다른 장점은 생성자와 다르게 객체를 생성할때 오류가 감지되면 상황에 따라서 null을 반환할 수 있다. 또한 아래 설명할 팩토리 메소드 패턴의 예시처럼 다형적으로 객체를 생성할 수 있다.

팩토리 메소드 패턴은 객체를 생성하는 팩토리 클래스를 **다형적으로** 구성함으로써 하위 클래스의 메소드가 상위 클래스의 팩토리 메소드를 오버라이딩하여 사용하는 패턴을 의미한다. 
...
```
각 나라에 따라 용량(ml)을 다르게 하고 싶으면 아래와 같이 추상 팩토리 메소드를 정의한 추상 클래스를 구현하고 나라마다 다형적으로 하위 클래스의 팩토리 메소드를 구성할 수 있다. 코드 예시는 다음과 같다.
...
전략 패턴과 유사하지만 전략을 별도의 클래스가 아닌 내부 클래스로 구현한 패턴이다. Spring DI는 템플릿 콜백 패턴을 사용한다. 앞서 전략 패턴에서 예로 들었던 군인 객체와 전략 대한 예시를 템플릿 콜백 패턴으로 변경했을때 코드는 다음과 같다.
